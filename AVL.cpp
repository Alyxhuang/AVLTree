...
#define Value data.second
 ...
 private:
  NodePointer rebuild34(NodePointer a, NodePointer b, NodePointer c, NodePointer T0, NodePointer T1, NodePointer T2,
                        NodePointer T3) {
    // 3 + 4 重构
  ...
  NodePointer reBalance(NodePointer v) {
    ...
    if (isLeftChild(p)) {
      ...
      if (isRightChild(v)) {
        ...
        v->father = g->father;
        if (g->father) isLeftChild(g) ? (g->father->left = v) : (g->father->right = v);
        return rebuild34(g, v, p, g->left, v->left, v->right, p->right);
      }
    }
  }
  bool isLeftChild(NodePointer cur) { return cur && cur->father && (cur->father->left == cur); }
  bool isRightChild(NodePointer cur) { return cur && cur->father && (cur->father->right == cur); }
  bool isLeaf(NodePointer cur) { return cur && (cur->left == nullptr) && (cur->right == nullptr); }
  ...
  bool isBalanced(NodePointer v) { return (v == nullptr) || (avlAbs(getHeight(v->left) - getHeight(v->right)) < 2); }
  NodePointer getHead() {
  ...
  int getHeight(NodePointer v) { return v == nullptr ? 0 : v->height; }
  int avlAbs(int a) { return a > 0 ? a : -a; }
